...
#abrir arquivos
data_chagas <- read.dbf("C:/Users/ADM/Dropbox/PC/Desktop/Scipts by_Silmara/Projeto Chagas/entradas/CHAGANET.dbf")
data_gal_soro <- read.csv("C:/Users/ADM/Dropbox/PC/Desktop/Scipts by_Silmara/Projeto Chagas/entradas/data igg.csv", header = TRUE, sep = ";")
data_gal_gota <- read.csv("C:/Users/ADM/Dropbox/PC/Desktop/Scipts by_Silmara/Projeto Chagas/entradas/data gota.csv", header = TRUE, sep = ";")


#ajustar datas
data_gal_soro$Data.de.Nascimento <- dmy(data_gal_soro$Data.de.Nascimento)
data_gal_gota$Data.de.Nascimento <- dmy(data_gal_gota$Data.de.Nascimento)
data_gal_soro$Data.do.1º.Sintomas <- dmy(data_gal_soro$Data.do.1º.Sintomas)
data_gal_gota$Data.do.1º.Sintomas <- dmy(data_gal_gota$Data.do.1º.Sintomas)

#ajustar datas
data_chagas$DT_SIN_PRI <- as.Date(data_chagas$DT_SIN_PRI, format="%d-%m-%Y")
data_chagas$DT_NASC <- as.Date(data_chagas$DT_NASC, format="%d-%m-%Y")
...
RemoveAcentos(data_chagas$NM_PACIENT)
RemoveAcentos(data_gal_soro$Paciente)
RemoveAcentos(data_gal_gota$Paciente)
...
data_chagas <- rename(data_chagas, cod_ibge = ID_MN_RESI)
data_chagas <- left_join(data_chagas, municipios_PB, by = "cod_ibge")

#criar chave
id_pessoa_chagas <- paste(data_chagas$NM_PACIENT,
                         data_chagas$DT_NASC,
                         data_chagas$cod_ibge)

id_pessoa_gal_igg <- paste(data_gal_soro$Paciente,
                           data_gal_soro$Data.de.Nascimento,
                           data_gal_soro$IBGE.Município.de.Residência)

id_pessoa_gal_gota <- paste(data_gal_gota$Paciente,
                           data_gal_gota$Data.de.Nascimento,
                           data_gal_gota$IBGE.Município.de.Residência)


#acrescentar coluna com ID_Pessoa
data_chagas <- mutate(data_chagas, id_pessoa_chagas)
data_gal_soro <- mutate(data_gal_soro, id_pessoa_gal_igg)
data_gal_gota <- mutate(data_gal_gota, id_pessoa_gal_gota)


# separar duplicidades
DT_data_chagas <- Sys.Date() - data_chagas$DT_SIN_PRI #usar para verificar encerramento
DT_data_chagas <- data.frame(DT_data_chagas) #usar para verificar encerramento
sum(duplicated(data_chagas$id_pessoa_chagas))

duplicidade_chagas <- data_chagas[duplicated(data_chagas$id_pessoa_chagas), ]
duplicidade_chagas <- subset(duplicidade_chagas, 
                            select = -c(id_pessoa_chagas)) 

#separar gal que não está notificado
data_gal_soro <- rename(data_gal_soro, id_pessoa_chagas = id_pessoa_gal_igg)
data_gal_gota <- rename(data_gal_gota, id_pessoa_chagas = id_pessoa_gal_gota)


#cruzar gal gota espessa e sorologia com sinan
#separar gal_gota que não está notificado
gal_gota_notificar_sinan <- anti_join(data_gal_gota, data_chagas,
                                 by = "id_pessoa_chagas")

gal_gota_notificar_sinan <- select (gal_gota_notificar_sinan,
                                   Requisição,
                                   Unidade.Solicitante,
                                   Data.do.1º.Sintomas,
                                   Paciente,
...
gal_gota_notificar_sinan <- data.frame(gal_gota_notificar_sinan)
gal_gota_notificar_sinan <- gal_gota_notificar_sinan[!duplicated(gal_gota_notificar_sinan), ]
gal_gota_notificar_sinan <- subset(gal_gota_notificar_sinan, 
                                  select = -c(id_pessoa_chagas)) 

#separar gal_soro que não está notificado
gal_soro_notificar_sinan <- anti_join(data_gal_soro, data_chagas,
                                     by = "id_pessoa_chagas")

gal_soro_notificar_sinan <- select (gal_soro_notificar_sinan,
                                           Requisição,
                                           Unidade.Solicitante,
                                           Data.do.1º.Sintomas,
                                           Paciente,
...
gal_soro_notificar_sinan <- data.frame(gal_soro_notificar_sinan)
gal_soro_notificar_sinan <- gal_soro_notificar_sinan[!duplicated(gal_soro_notificar_sinan), ]
gal_soro_notificar_sinan <- subset(gal_soro_notificar_sinan, 
                                          select = -c(id_pessoa_chagas)) 

#foi para o sinan, mas não foi para o gal
sinan_sem_gal_gota <- anti_join(data_chagas, data_gal_gota,
                               by = "id_pessoa_chagas")

sinan_sem_gal_soro <- anti_join(data_chagas, data_gal_soro,
                               by = "id_pessoa_chagas")

#notificado no sinan, sem gal

sinan_sem_gal <- bind_rows (sinan_sem_gal_gota, sinan_sem_gal_soro)
sinan_sem_gal <- sinan_sem_gal[!duplicated(sinan_sem_gal), ]
sinan_sem_gal <- subset(sinan_sem_gal, 
                                   select = -c(id_pessoa_chagas))

#separar notificação que tem resultado, porém não está preenchido
data_chagas_sem_resul0 <- data_chagas %>% filter(is.na(data_chagas$EXAME))
data_chagas_sem_resul1 <- data_chagas %>% filter(is.na(data_chagas$IMU_IGM_S1))
data_chagas_sem_resul2 <- data_chagas %>% filter(is.na(data_chagas$IMU_IGM_S2))
data_chagas_sem_resul3 <- data_chagas %>% filter(is.na(data_chagas$IMU_IGG_S1))
data_chagas_sem_resul4 <- data_chagas %>% filter(is.na(data_chagas$IMU_IGG_S2))


data_chagas_sem_resultado <- bind_rows(data_chagas_sem_resul0, data_chagas_sem_resul1,
                                      data_chagas_sem_resul2, data_chagas_sem_resul3,
                                      data_chagas_sem_resul4)

data_chagas_sem_resultado <- distinct(data_chagas_sem_resultado)
data_chagas_sem_resultado <- data.frame(data_chagas_sem_resultado)


data_chagas_inserir_resultado_gal_gota <- left_join(data_gal_gota, data_chagas_sem_resultado, by = "id_pessoa_chagas")
data_chagas_inserir_resultado_gal_gota <- select (data_chagas_inserir_resultado_gal_gota,
                                             Requisição,
                                             Data.do.1º.Sintomas,
                                             Paciente,
                                             Data.de.Nascimento,
...
data_chagas_inserir_resultado_gal_soro <- left_join(data_gal_soro, data_chagas_sem_resultado, by = "id_pessoa_chagas")
data_chagas_inserir_resultado_gal_soro <- select (data_chagas_inserir_resultado_gal_soro,
                                                 Requisição,
                                                 Data.do.1º.Sintomas,
                                                 Paciente,
                                                 Data.de.Nascimento,
                                                 ...
                                                 DT_NASC,
                                                 nm_municipios_PB)
data_chagas_inserir_resultado_gal_soro <- data_chagas_inserir_resultado_gal_soro[!duplicated(data_chagas_inserir_resultado_gal_soro), ]

#separar inconsistência de evolução
data_chagas <- mutate(data_chagas, DT_data_chagas)

data_chagas_evo_vazia <- data_chagas %>% filter(is.na(data_chagas$EVOLUCAO))
data_chagas_corrigir_evolucao1 <- filter(data_chagas,
                                        data_chagas$EVOLUCAO != 1,
                                        data_chagas$EVOLUCAO != 2,
                                        data_chagas$EVOLUCAO != 3)
data_chagas_corrigir_evolucao2 <- filter(data_chagas,
                                        data_chagas$EVOLUCAO == 9)

data_chagas_corrigir_evolucao3 <- filter(data_chagas,
                                        data_chagas$DT_data_chagas > '46 days',
                                        data_chagas$EVOLUCAO != 1,
                                        data_chagas$EVOLUCAO != 2,
                                        data_chagas$EVOLUCAO != 3)

data_chagas_corrigir_evolucao <-  bind_rows(data_chagas_evo_vazia, 
                                           data_chagas_corrigir_evolucao1,
                                           data_chagas_corrigir_evolucao2,
                                           data_chagas_corrigir_evolucao3)

data_chagas_corrigir_evolucao <- data.frame(data_chagas_corrigir_evolucao) 
data_chagas_corrigir_evolucao <- data_chagas_corrigir_evolucao[!duplicated(data_chagas_corrigir_evolucao), ]
...
data_chagas_obito_sem_class <- data_chagas %>% filter(is.na(data_chagas$CLASSI_FIN) & 
                                                      data_chagas$EVOLUCAO == 2)
data_chagas_obito_sem_class <- subset(data_chagas_obito_sem_class, 
                                     select = -c(id_pessoa_chagas)) 
data_chagas_obito_sem_class <- subset(data_chagas_obito_sem_class, 
                                     select = -c(DT_data_chagas))
data_chagas_obito_sem_class <- data_chagas_obito_sem_class[!duplicated(data_chagas_obito_sem_class), ]


data_outro_obito_sem_class <- data_chagas %>% filter(is.na(data_chagas$CLASSI_FIN) & 
                                                      data_chagas$EVOLUCAO == 3)
...
export(list(duplicidades = duplicidade_chagas,
            ...
            gal_soro_notificar_sinan = gal_soro_notificar_sinan,
            sinan_sem_gal_gota = sinan_sem_gal_gota,
            sinan_sem_gal_soro = sinan_sem_gal_soro,
            sinan_sem_gal = sinan_sem_gal,
            inserir_resultado_gal_gota = data_chagas_inserir_resultado_gal_gota,
            inserir_resultado_gal_soro = data_chagas_inserir_resultado_gal_soro,
            corrigir_evolucao = data_chagas_corrigir_evolucao,
            inserir_resul_gal_soro = data_chagas_inserir_resultado_gal_soro,
